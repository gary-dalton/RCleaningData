setwd("~/R/RCleaningData/UCI HAR Dataset")
## Read in the features table
## Adjust features table to include only those columns that include mean() or std()
f_r<- read.table("features.txt", col.names=c("col_num", "col_name" ), stringsAsFactors=FALSE)
...
##
tmp<- read.table("test/X_test.txt")
tmp_data<-tmp[,features$col_num]
colnames(tmp_data)<- features$col_name
tmp_sub<- read.table("test/subject_test.txt", col.names=c("subject"))
tmp_act<- read.table("test/y_test.txt", col.names=c("activity"))
...
tmp_data<-tmp[,features$col_num]
colnames(tmp_data)<- features$col_name
tmp_sub<- read.table("train/subject_train.txt", col.names=c("subject"))
tmp_act<- read.table("train/y_train.txt", col.names=c("activity"))
tmp_act[,'activity']<- factor(tmp_act[,'activity'],  labels=c("WALKING", "WALKING_UPSTAIRS", "WALKING_DOWNSTAIRS", "SITTING", "STANDING", "LAYING"))
